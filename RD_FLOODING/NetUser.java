...
class NetUser extends GridUser
{
    ...
    public void body()
    {
        ...
        // neighbor assignment
        int [][] neighborRecord = new int [num_GIS] [num_GIS];
        for (i = 0; i < num_GIS; i++)
        {
            for (j = 0; j < num_GIS; j++)
                neighborRecord[i][j] = 0;
        }
 ...
 */
            ...
            int floodGIS;
            int floodCounter = 0;
            floodAdjacency = extractGISAdjacency(adjacencyMatrix, localGISIndex);
            floodGIS = localGISIndex;
            
            GISAdjacency = extractGISAdjacency(adjacencyMatrix, localGISIndex);
            nextGIS = localGISIndex;

            while (this.gridletStatus[i] != 1 && sentinel != 1 && this.hopReq[i] <=maxHop)
            {
                //previousGIS = nextGIS;
                for (j = 0; j < floodAdjacency.length; j++)
                {
                    nextGIS = floodAdjacency[j];
                    ...
                    if (isVisit [nextGIS] != 1)
                    {
                        ...
                        for (k = 0; k < memberRsource.length; k++ )
                        {

                            resChar = (ResourceCharacteristics) resCharList.get(memberRsource[k]);
                            if ( resChar.getNumPE() == gl.getNumPE())
                            {
                                success = super.gridletSubmit(gl,resChar.getResourceID(),0.0,true);
                                // calculate resource ID
                                ...
                                //for (k = 0; k < GISAdjacency.length; k++)
                                //neighborRecord [previousGIS][previousGIS]++;
     ...
     */
                ...
                if ( this.gridletStatus[i] == 1)
                    break;
                floodAdjacency = extractGISAdjacency(adjacencyMatrix,floodGIS);
                if (floodCounter < floodAdjacency.length)
                {
                    floodAdjacency = extractGISAdjacency(adjacencyMatrix, floodAdjacency[floodCounter]);
                    floodCounter++;
                }
                else
                {
                    floodGIS = floodAdjacency[0]; 
                    floodCounter = 0;
                }
...
/*
        ...
        {
            for (j = 0; j < num_GIS; j++)
                System.out.print(neighborRecord[i][j] + " ");
            System.out.println();
        }
*/        
        ////////////////////////////////////////////////////////
