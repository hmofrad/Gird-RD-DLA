...
package RD_BESTNEIGHBOR;


/*
 * Author: Mohammad Hasanzadeh 
 * Date: October 2012
 * Description: A program in which execute as a grid User
 *
 *
 *
 */
        ...
        for (i = 0; i < totalGridlet; i++) // Process Gridlets
        {
            ...
            for (j = 0; j < memberRsource.length; j++ )
            {
                ...
                if ( resChar.getNumPE() == gl.getNumPE())
                {
                    ...
                    this.successReq++;
                    neighborRecord [localGISIndex][localGISIndex]++;
 ...
 */
            ...
            {
                previousGIS = nextGIS;
                GISAdjacency = extractGISAdjacency(adjacencyMatrix, previousGIS);
                bestGIS = 0;
                bCounter = 0;
                for (k = 0; k < GISAdjacency.length; k++)
                {
                    if (neighborRecord[previousGIS][GISAdjacency[k]] > 0)
                    {
                        if(neighborRecord[previousGIS][GISAdjacency[k]] > bestGIS)
                            bestGIS = neighborRecord[previousGIS][GISAdjacency[k]];
                    }
                    else
                    {
                        bCounter++;
                ...
                if (bCounter ==  GISAdjacency.length)
